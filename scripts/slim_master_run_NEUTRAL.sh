#---------------------------
# SLIM MASTER SCRIPT - Modified to only run neutral simulations
...
sed "s/selection_coef/${SEL_COEF}/g" $MASTER/scripts/1_Chunk_Spatial_Slim_Neutral.txt > 1_Chunk_Slim_mod_demographics_${MOAB_JOBARRAYINDEX}
...
sed "s/selection_coef/${SEL_COEF}/g" $MASTER/scripts/1.5_Chunk_Spatial_Slim_Neutral.txt > 1.5_Chunk_Slim_mod_demographics_${MOAB_JOBARRAYINDEX}
...
BURN_GEN=$((5*2*POP_N))
cp $MASTER/scripts/3_Chunk_Spatial_Slim_FIRSTHALF_neutral.txt 3_Chunk_Slim_mod_SIM_FIRST_HALF_${MOAB_JOBARRAYINDEX}
cp $MASTER/scripts/3.5_Chunk_Spatial_Slim_SECONDHALF_neutral.txt 3.5_Chunk_Slim_mod_SIM_SECOND_HALF_${MOAB_JOBARRAYINDEX}
...
cat $(ls GENE_${MOAB_JOBARRAYINDEX}_*.txt) > GENE_${MOAB_JOBARRAYINDEX}_allTreatments_SLIM_neutral.out

# Add a header
echo -e "Gene_ID\tIteration\tRun_ID\tSelection_Coef\tBottleneck\tPop2_Size\tMigration\tGene_Size\tGeneration\tFST\tdXY\tPop1_PI\tPop2_PI\tExon_Length_Total" | cat - GENE_${MOAB_JOBARRAYINDEX}_allTreatments_SLIM_neutral.out > tmp && mv tmp GENE_${MOAB_JOBARRAYINDEX}_allTreatments_SLIM_neutral.out
mv GENE_${MOAB_JOBARRAYINDEX}_allTreatments_SLIM_neutral.out $MASTER/demographics_model/$OUT_DIR/
sed -i '/^$/d' $MASTER/demographics_model/$OUT_DIR/GENE_${MOAB_JOBARRAYINDEX}_allTreatments_SLIM_neutral.out

# Kill the folder
rm -rf $MASTER/demographics_model/$OUT_DIR/GENE_${MOAB_JOBARRAYINDEX}
