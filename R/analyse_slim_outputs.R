...
# This is the main script for handling outputs

# STEP 1 - Correlations
# STEP 2 - Distributions
# STEP 3 - FPR/FNR
# STEP 4 - Overlapping outliers
# STEP 5 - LMM of measures
##########
...
# Separate out Nulls
slim_null<-slim_dd[slim_dd$run_type == "Null",]
slim_neutral<-slim_dd[slim_dd$run_type == "Neutral",]
slim_dd<-slim_dd[slim_dd$run_type == "True",]
...
FPR_analysis<-lapply(generations_to_plot,function(gen){
  ...
  # Change y-axes
  FPR_sub[FPR_sub$run_type=="True","run_type"]<-"Divergent"
  FPR_sub[FPR_sub$run_type=="Null","run_type"]<-"Stabilising"
  quantile_list[quantile_list$run_type=="True","run_type"]<-"Divergent"
  quantile_list[quantile_list$run_type=="Null","run_type"]<-"Stabilising"
...
# Plot them and save
pdf(paste0(fig_out,"/Evolv_gen_by_demography.pdf"),height=6,width=14)
...
summary(lmm_fixed)
r.squaredGLMM(lmm_fixed) # Variance explained
...
Fst_selection_fig<-ggplot(Fst_selection_plot,aes(Selection_Coef,FST))+
  ...
  theme(axis.text = element_text(size=18),
        axis.title = element_text(size=20),
        strip.text = element_text(size=18))+
  labs(y=expression(F[ST]),x=expression(Selection~Coefficient))
...
summary(lmm2_fixed)
r.squaredGLMM(lmm2_fixed) # Variance explained
