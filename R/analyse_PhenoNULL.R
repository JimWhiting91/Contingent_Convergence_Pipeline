...
slim_dd<-slim_dd[slim_dd$run_type == "Null",]
...
treatment_vec<-c("Bottleneck","Pop2_Size","Migration")
treatment_vec2<-c("Bottleneck","DP Pop Size","Migration")
...
evolv_gen_list<-lapply(1:length(treatment_vec2),function(x){
  ...
  g1<-ggplot(data = plot_dd, aes(x = as.factor(plot_dd[,treatment_vec[x]]), y = (Evolv_Gen),fill=as.factor(plot_dd[,treatment_vec[x]]))) +
    ...
    labs(x=treatment_vec2[x],y="Generation Phenotype Reached")
  
  return(list(tmp,tmp2,g1))
})

# Plot them and save
pdf(paste0(fig_out,"/Evolv_gen_by_demography.pdf"),height=4,width=14)
...
lmm1<-lmer(FST~Selection_Coef+log(exon_N)+log(Target_sel)+log(gene_length)+(1|Gene_ID)+(1|Treatment_Demo),slim_dd_GEN4[slim_dd_GEN4$Migration == 0.002 &
...
lmm2<-lmer(FST~Selection_Coef+log(exon_N)+log(Target_sel)+log(gene_length)+(1|Gene_ID)+(1|Treatment_Demo),slim_dd_GEN4[slim_dd_GEN4$Migration == 0&
                                                                                                                                        slim_dd_GEN4$Evolv_Gen > 0,])
drop1(lmm2)
...
lmm3<-lmer(dXY~Selection_Coef+log(exon_N)+log(Target_sel)+log(gene_length)+(1|Gene_ID)+(1|Treatment_Demo),slim_dd_GEN4[slim_dd_GEN4$Migration == 0.002 &
                                                                                                                                        slim_dd_GEN4$Evolv_Gen > 0,])
...
summary(lmm3_fixed)
r.squaredGLMM(lmm3_fixed)
# Selection has strongest effect but also strong effect of target of selection

######################################################
# DXY now for treatments WITHOUT migration
lmm4<-lmer(dXY~Selection_Coef+log(exon_N)+log(Target_sel)+log(gene_length)+(1|Gene_ID)+(1|Treatment_Demo),slim_dd_GEN4[slim_dd_GEN4$Migration == 0 &
                                                                                                                                                  slim_dd_GEN4$Evolv_Gen > 0,])
drop1(lmm4)
...
lmm5<-lmer(deltaPI~Selection_Coef+log(exon_N)+log(Target_sel)+log(gene_length)+(1|Gene_ID)+(1|Treatment_Demo),slim_dd_GEN4[slim_dd_GEN4$Migration == 0.002 &
                                                                                                                                            slim_dd_GEN4$Evolv_Gen > 0,])
...
summary(lmm5_fixed)
r.squaredGLMM(lmm5_fixed)
# Selection has strongest effect but also strong effect of target of selection

######################################################
# deltaPI now for treatments WITHOUT migration
lmm6<-lmer(deltaPI~Selection_Coef+log(exon_N)+log(Target_sel)+log(gene_length)+(1|Gene_ID)+(1|Treatment_Demo),slim_dd_GEN4[slim_dd_GEN4$Migration == 0 &
                                                                                                                                            slim_dd_GEN4$Evolv_Gen > 0,])
drop1(lmm6)
model.step6<-step(lmm6,scope=~.^2,direction="both")
lmm6_fixed<-lmer(deltaPI ~ Selection_Coef + (1 | Gene_ID) + (1 | Treatment_Demo),slim_dd_GEN4[slim_dd_GEN4$Migration == 0 &
                                                                                                                 slim_dd_GEN4$Evolv_Gen > 0,])
# Evaluate
drop1(lmm6_fixed)
